...
int main(void) {
	...
	if (SDL_SetColorKey(img, 1, SDL_MapRGB(img->format, 0, 255, 0)) != 0) {
		printf("ERROR: failed to set color key: %s\n", SDL_GetError());
		SDL_FreeSurface(img);
		return 1;
	}
	//second value is boolean.
	//SDL_SRCCOLORKEY == 1  insted of the "1" entered in SDL_SetColorKey

	//Third value is color

	while (!quit) {
		/* Clear window to BLACK*/
		if (SDL_FillRect(w,NULL,50) != 0) {
			printf("ERROR: failed to draw rect: %s\n", SDL_GetError());
			break;
		}
		//the third argument is the color and It can be negative :)
		//(-1) is White and 1 is black


		/* Green rectangle button*/
		if (SDL_FillRect(w, &rect, SDL_MapRGB(w->format, 0, 0, 255)) != 0) {
			printf("ERROR: failed to draw rect: %s\n", SDL_GetError());
			break;
		}
		...
		if (SDL_BlitSurface(img, &imgrect, w, &rect) != 0) {
			SDL_FreeSurface(img);
...
/*		 Advance to next sprite
		...
		if (imgrect.y >= img->h) {
			imgrect.y = 0;
			imgrect.x += imgrect.w;
			if (imgrect.x >= img->w) imgrect.x = 0;
		}*/
